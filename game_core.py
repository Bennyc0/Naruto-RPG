import time
import random
import # Look into multiprocessing

# The code is bit scuffed together this is itteration 1

class entity:
    # Constuctor the blueprints for creating a class
    def __init__(self, name, level = 100, health = random.randint(5,10), attack = random.randint(1,3), defence = random.randint(1,3), skill = ["Punch"]):
        self.name = name
...
def fight(entity1, entity2):
    ...
    if __name__ == '__main__':
        Thread(target = entity1.damage, args=(entity2).start()
        Thread(target = entity2.damage, args=(entity1).start()
    
mob1 = entity("Itachi", 10, 2, 2, ["Punch", "FireBall"])
